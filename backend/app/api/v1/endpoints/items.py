...
def delete_item_endpoint(item_id: int, db: Session = Depends(get_db)):
    """
    Удалить товар по ID (Для Админа). 
    Возвращает 204 No Content при успехе.
    """
    # ⚠️ Убедитесь, что 'crud_item' импортирован правильно в этом файле
    from app.crud import item as crud_item # ПРОВЕРЬТЕ ПУТЬ
    
    success = crud_item.delete_item(db, item_id=item_id)
    
    if not success:
        # Если crud_item.delete_item вернул False
...
def read_all_items_admin(db: Session = Depends(get_db), skip: int = 0, limit: int = 100):
    """
    Получить список ВСЕХ товаров, включая неактивные (только для Админа).
    """
    # ⚠️ Убедитесь, что crud_item импортирован правильно
    from app.crud import item as crud_item # ПРОВЕРЬТЕ ПУТЬ
    
    # 💡 Предполагается, что у вас есть функция get_all_items в CRUD
    db_items = crud_item.get_all_items(db, skip=skip, limit=limit)
    
    # Применяем функцию, которая добавляет данные категории и список URL (если используется)
    return [_add_category_to_item(item) for item in db_items]