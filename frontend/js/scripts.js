...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const PRICE_SURCHARGE = {
        'cash': 0.00,                 // Наличные: 0%
        'qr': 0.06,                   // QR код: +6%
        'card': 0.07,                 // Карта/Терминал: +7%
        'credit_installments': 0.05   // Кредит/Рассрочка: +5%
    };
    /**
    ...
    function saveCartToStorage() {
        localStorage.setItem('userShoppingCart', JSON.stringify(cartItems));
    }
    ...
    // ✅ НОВАЯ ПРОВЕРКА
        ...
        if (price === null || price === undefined || isNaN(numericPrice) || numericPrice < 0) { 
    ...
    /**
    * Рассчитывает общую сумму корзины с учетом наценки за способ оплаты.
    * @returns {object} { baseTotal: number, finalTotal: number, surchargeRate: number, method: string }
    */
    ...
    // 1. Вывод списка товаров и расчет BASE Total (цены без комиссии)
        cartItems.forEach(item => { 
            ...
            itemLine.innerHTML = `<span>${item.name}${optionsHtml}</span><span class="item-price">${formatPrice(price)}</span>`; 
            checkoutItemsSummary.appendChild(itemLine); 
            baseTotal += price; 
        }); 
    
    // 2. 🔥 НОВАЯ ЛОГИКА: Расчет с учетом наценки
        const calculation = calculateFinalTotal();

        let surchargeInfo = '';
        if (calculation.surchargeRate > 0) {
            const surchargeAmount = calculation.finalTotal - calculation.baseTotal;
            const methodDisplay = calculation.method === 'card' ? 'Карта/Терминал' : 
                                calculation.method === 'qr' ? 'QR код' : 
                                'Кредит/Рассрочка';
                              
            surchargeInfo = `<p class="surcharge-info">Наценка за *${methodDisplay}*: **${formatPrice(surchargeAmount)}**</p>`;
        
        // Отображение базовой суммы перед наценкой
            checkoutItemsSummary.insertAdjacentHTML('beforeend', `<p class="base-total-info">Базовая сумма: ${formatPrice(calculation.baseTotal)}</p>`);
        }

        checkoutItemsSummary.insertAdjacentHTML('beforeend', surchargeInfo);
    
    // 3. Отображаем ИТОГОВУЮ сумму с наценкой
        checkoutTotalPriceEl.textContent = formatPrice(calculation.finalTotal); 
    }
    ...
    // Если дропдаун открывается, рендерим категории
        if (dropdown.classList.contains('visible')) {
        // ИСПРАВЛЕНИЕ: Добавьте 's'
            renderCategoriesDropdown(); 
        }
    }

    function renderCategoriesDropdown() {
        if (!categoriesListEl || !dropdownLoadingMessage) return;

        categoriesListEl.innerHTML = '';

        // Используем НОВУЮ переменную nestedCategories
        if (nestedCategories.length === 0) {
            dropdownLoadingMessage.textContent = 'Категории не загружены.';
            dropdownLoadingMessage.classList.remove('hidden');
            return;
        }

        dropdownLoadingMessage.classList.add('hidden');

        // 1. "Все товары" (как и было)
        ...
        // 2. Рекурсивная функция для создания элементов
        function createCategoryElement(category) {
            ...
            if (hasSubcategories) {
                listItem.className = 'category-item has-submenu';
                // Добавляем контент с иконкой
                listItem.innerHTML = `
                    <span>${category.name}</span>
                    <i class="fa-solid fa-chevron-right submenu-toggle-icon"></i>
                `;
                ...
                // Рекурсивно заполняем вложенный список
                category.subcategories.forEach(subCat => {
                    submenu.appendChild(createCategoryElement(subCat)); // Рекурсия!
                });
                
                listItem.appendChild(submenu);
                
                // Добавляем слушатель на родителя (на саму <li>)
                listItem.addEventListener('click', (e) => {
                    // Останавливаем всплытие, чтобы клик не фильтровал по родительской категории
                    e.stopPropagation(); 
                    // Переключаем класс 'open' для анимации
                    listItem.classList.toggle('open');
                });

            } else {
                // Это обычный элемент (или подкатегория)
                ...
                listItem.addEventListener('click', (e) => {
                    e.stopPropagation(); // Важно, чтобы не "схлопнуть" родителя
                    showHomePage();
        ...
        // 3. Запускаем рекурсию для категорий верхнего уровня
        nestedCategories.forEach(category => {
            categoriesListEl.appendChild(createCategoryElement(category));
        });
    }
    // --------------------------------------------------------------------------------
    // --- КОРЗИНА ---
    ...
    // 🌟 ИСПРАВЛЕНО: используем image_urls (массив)
        ...
        card.innerHTML = `
            ...
            <div class="product-price-row">
                <span class="product-price">${(displayPrice > 0) ? 'от ' : ''}${formattedPrice}</span>
    ...
    function updatePriceAndButton() { 
        ...
        // 5. 🌟 ОБНОВЛЕНИЕ ИЗОБРАЖЕНИЯ (НОВЫЙ БЛОК)
        // Это предполагает, что каждый вариант товара (item)
        // содержит в item.image_urls[0] картинку,
        // соответствующую его цвету (item.color)
        ...
        if (imgEl) {
            ...
            if (selectedOptions.color) {
                // 1. Ищем ЛЮБОЙ вариант, соответствующий выбранному цвету
                const variantForImage = currentItemVariants.find(item => item.color === selectedOptions.color);
                
                if (variantForImage && variantForImage.image_urls && variantForImage.image_urls.length > 0) {
                    // Используем первую картинку этого варианта
                    imageUrlToSet = variantForImage.image_urls[0];
                }
            }

            // 2. Если цвет не выбран ИЛИ у варианта с этим цветом нет картинки
            if (!imageUrlToSet) {
                // ...возвращаемся к картинке по умолчанию (от самого первого варианта в списке)
                const defaultItem = currentItemVariants[0];
                if (defaultItem && defaultItem.image_urls && defaultItem.image_urls.length > 0) {
                    imageUrlToSet = defaultItem.image_urls[0];
                }
            }
            
            // 3. Если картинка так и не найдена, ставим заглушку
            imgEl.src = imageUrlToSet || PLACEHOLDER_IMAGE;
        }
    }
    ...
    // scripts.js (Найдите и замените функцию renderProductDetail)
    ...
    function renderProductDetail(baseName) { 
        ...
        // Получаем описание
        const description = displayItem.description || 'Описание отсутствует.';

        
        if (productDetailContainer) { 
            productDetailContainer.innerHTML = `
                <div class="product-details-header">
                    <img src="${imageUrl}" alt="${baseName}" class="product-image" id="detail-product-image">
                ...
                <div class="detail-footer">
                    <div class="detail-price-box">
                        ...
                        <span class="price-note">Цена указана за наличный расчёт</span>
        ...
        // Принудительное обновление, чтобы корректно отобразить начальное состояние
        updatePriceAndButton();
        ...
        } // ❗ ИСПРАВЛЕНИЕ: ЗАКРЫВАЕТ if (finalAddBtn)
    } // ❗ ИСПРАВЛЕНИЕ: ЗАКРЫВАЕТ function renderProductDetail
    // --------------------------------------------------------------------------------
    ...
    // 2. Рендеринг секций товаров
        ...
        // - Если фильтра нет (главная страница), показываем только первые 2.
            const productsToShow = isFilteredView ? baseItemsArray : baseItemsArray.slice(0, 2); // ❗ ИЗМЕНЕНО: 3 -> 2
        
        // Кнопка "Все >" нужна, только если:
        // 1. Товаров в категории > 2 (baseItemsArray.length > productsToShow.length)
        // 2. Мы НЕ находимся в режиме фильтрации (не показаны все товары)
            const shouldShowViewAll = baseItemsArray.length > 2 && !isFilteredView; // ❗ ИЗМЕНЕНО: 3 -> 2
    ...
    async function loadAndRenderProducts() {
        ...
        try {
            ...
            const nestedCategoriesData = await categoriesResponse.json(); // 1. Переименовали

            if (!Array.isArray(itemsData)) throw new Error("API /items/ вернул не массив");
            if (!Array.isArray(nestedCategoriesData)) throw new Error("API /categories/ вернул не массив"); // 2. Проверяем новую
            ...
            function flattenCategories(categories) {
                ...
                categories.forEach(cat => {
                    // Добавляем родителя (без детей, чтобы не путать)
            ...
            // allCategories - теперь плоский список для поиска
            allCategories = flattenCategories(nestedCategoriesData); 
            
            // nestedCategories - сохраняем вложенную структуру для дропдауна
            nestedCategories = nestedCategoriesData;
    ...
    function setupBannerSlider() {
        ...
        // 1. Сдвиг "ленты" (на 0% или -50% для двух слайдов)
    ...
    // 3. Запуск автоматической прокрутки
        let autoSlideTimer = setInterval(() => {
            showSlide(currentSlide + 1);
        }, slideInterval);

    // 4. (Опционально) Нажатие на точки для ручного переключения
    // (также сбрасывает таймер, чтобы было удобнее)
        dots.forEach(dot => {
    ...
    function setupGlobalListeners() {
        ...
        // 1. Слушатель 'input' для поля поиска
        // Показывает/скрывает кнопку очистки при наборе и запускает поиск
            ...
            // Вызываем существующую функцию поиска/фильтрации
                handleSearch();
            });

        // 2. Слушатель 'click' для кнопки очистки
            clearSearchBtn.addEventListener('click', () => {
                searchInput.value = ''; // Очищаем поле ввода
                clearSearchBtn.classList.add('hidden'); // Скрываем крестик
                searchInput.focus(); // Возвращаем фокус для удобства
            
            // Вызываем handleSearch(), чтобы вернуться к полному списку товаров
                handleSearch(); 
            });
        }
        ...
        document.addEventListener('click', (e) => {
            const dropdownWrapper = document.querySelector('.catalog-dropdown-wrapper');
            const isClickOutside = dropdownWrapper && !dropdownWrapper.contains(e.target);
    ...
    // 1. Слушатель для пересчета суммы при смене способа оплаты
    document.querySelectorAll('input[name="payment_method"]').forEach(radio => {
    // При изменении способа оплаты перерисовываем итоговую сумму
        radio.addEventListener('change', renderCheckoutSummary);
    });
    // 2. Слушатели для полей формы (показываем/скрываем адрес)
// Эта логика была в закомментированном блоке, но она нужна для формы:
    document.querySelectorAll('input[name="delivery_method"]').forEach(radio => {
        radio.addEventListener('change', (e) => {
            const addressGroup = document.getElementById('address-group');
            const addressField = document.getElementById('address');
...
//             // Если категорий нет, показываем сообщение об ошибке
//             dropdownLoadingMessage.textContent = 'Категории не загружены.';
//             dropdownLoadingMessage.classList.remove('hidden'); // Показываем P-тег
//             return; 
//         } 
        
//         // Если категории ЕСТЬ, скрываем сообщение
//         dropdownLoadingMessage.classList.add('hidden');
